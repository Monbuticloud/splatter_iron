        ...
        if let Some(tex) = &self.canvas.rendered_layers {
            ...
            if self.show_brush_preview {
                if let Some(hover_pos) = response.hover_pos() {
                    ...
                    let preview_start_x = pixel_x.saturating_sub(half) as f32;
                    let preview_end_x = (pixel_x + half).min(self.canvas.width - 1) as f32 + 1.0;
                    let preview_start_y = pixel_y.saturating_sub(half) as f32;
                    let preview_end_y = (pixel_y + half).min(self.canvas.height - 1) as f32 + 1.0;

                    // Map to screen space using the scale factor
                    let screen_x = response.rect.min.x + preview_start_x * (draw_size.x / self.canvas.width as f32);
                    let screen_y = response.rect.min.y + preview_start_y * (draw_size.y / self.canvas.height as f32);
                    let screen_w = (preview_end_x - preview_start_x) * (draw_size.x / self.canvas.width as f32);
                    let screen_h = (preview_end_y - preview_start_y) * (draw_size.y / self.canvas.height as f32);

                    let preview_rect = Rect::from_min_size(Pos2::new(screen_x, screen_y), egui::vec2(screen_w, screen_h));
                    ...
                    // Outline
                    ui.painter().rect_stroke(preview_rect, 0.0, egui::Stroke::new(1.0, self.current_color), StrokeKind::Middle);
                }
            }
