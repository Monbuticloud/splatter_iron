...
fn push_undo_makes_undo_available() {
    ...
    let record = canvas::draw_square(0, 0, 3, 3, &mut canvas, red(), 0);
...
fn push_undo_truncates_redo_stack() {
    ...
    let r2 = canvas::draw_square(4, 4, 7, 7, &mut canvas, red(), 0);
    hist.push_undo(r2);
...
fn clear_resets_history() {
    let mut hist = UndoHistory::new(100);
    let mut canvas = small_canvas();
    hist.push_undo(canvas::draw_square(0, 0, 3, 3, &mut canvas, red(), 0));
    hist.push_undo(canvas::draw_square(4, 4, 7, 7, &mut canvas, red(), 0));
    assert!(hist.can_undo());
    hist.clear();
    assert!(!hist.can_undo());
    assert!(!hist.can_redo());
